# Dice Chess Bot — Deno Random — AI Agent Guidelines

## Architecture Overview

- **Domain**: Production Dice Chess ladder anchor running the engine's unmodified `random` search on Deno
  Deploy. This is `anchor/random` from Anchor Set v1.0 — the lower horizon of the rating scale.
- **Contract**: Pure webhook endpoint implementing WebCrypto HMAC-SHA256 verification and ownership handshake.
  ...
  WebCrypto.
- **Core Rule**: Do not modify or optimize the strategy. `random` is deliberately fixed: it is the only anchor
  that engine speed-ups cannot strengthen at all, which is precisely its value.

## Developer Workflows

- **Setup**: `deno --version` (Deno 2.x; no install step — dependencies resolve from `deno.json`).
- **Tests**: `deno task test` (HMAC verification, handshake, replay window, and the strategy).
